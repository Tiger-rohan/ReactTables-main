import {
    ...
    PopoverContent, PopoverTrigger, Text, VStack
        ...
        ()=>{
            setColumnFilters(
                prev=>{
                    ...
                    return prev.map(
                        f=>f.id==='status' ? {...f,value:isActive 
                            ? statuses.filter(s=>s!==status.id) : statuses.concat(status.id),} : f)
                }
...
const FilterPopOver = ({columnFilters, setColumnFilters}) => {
  ...
  <PopoverTrigger>
    ...
    size="sm" 
    leftIcon={
    <Icon as={FilterIcon} fontSize={18}/>}>Filter</Button>
  </PopoverTrigger>
  <PopoverContent>
    <PopoverArrow />
    <PopoverCloseButton />
